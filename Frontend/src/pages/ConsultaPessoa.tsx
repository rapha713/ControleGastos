...
function ConsultaPessoas() {
  const [pessoas, setPessoas] = useState<consultaPessoa[]>([]);

  const [campoBusca, setCampoBusca] = useState("id");
  const [filtroTexto, setFiltroTexto] = useState("");
  const [ordenacao, setOrdenacao] = useState<{ campo: keyof consultaPessoa,
  ...
  const carregarPessoas = async () => {
    try {
      const response = await axios.get<consultaPessoa[]>(`${API_URL}/consultaPessoas`);
  ...
  const dadosProcessados = useMemo(() => {
        ...
        if (filtroTexto) {
          resultado = resultado.filter(p => {
            ...
            valorParaFiltrar = String(p[campoBusca as keyof consultaPessoa] || "");
        ...
        resultado.sort((a, b) => {
          const valA = a[ordenacao.campo as keyof consultaPessoa] ?? "";
          const valB = b[ordenacao.campo as keyof consultaPessoa] ?? "";
  ...
  return (
    <div className="pessoaMenu">
      ...
      <div className="pessoa">
        <h1>Consulta de totais por pessoa</h1>
        ...
        <table className="tabela">
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort('totalReceita')} style={{ cursor: 'pointer' }}>Total de Receitas {renderSeta('totalReceita')}</th>
